    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50/50 flex items-center justify-center py-12 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 bg-brand-blue rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-brand-blue/30">
            <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
            </svg>
          </div>
          <h1 className="text-2xl font-extrabold text-gray-900">Buat Akun Baru</h1>
          <p className="text-sm text-gray-400 mt-1">Bergabung dengan El Top Up sekarang</p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-7">
          ...
          >
            ...
            <span>{isGoogleLoading ? 'Mengarahkan ke Google...' : 'Daftar dengan Google'}</span>
          </button>

          {/* Divider */}
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 h-px bg-gray-100" />
            <span className="text-xs font-semibold text-gray-400 flex-shrink-0">atau daftar dengan email</span>
            <div className="flex-1 h-px bg-gray-100" />
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            {/* Name */}
            <div>
              <label htmlFor="reg-name" className="block text-sm font-semibold text-gray-600 mb-1.5">Nama (opsional)</label>
              <input
                id="reg-name"
                type="text"
                value={name}
            ...
            {/* Email */}
            <div>
              <label htmlFor="reg-email" className="block text-sm font-semibold text-gray-600 mb-1.5">Email</label>
              <input
                id="reg-email"
                type="email"
                value={email}
            ...
            {/* Password */}
            <div>
              <label htmlFor="reg-password" className="block text-sm font-semibold text-gray-600 mb-1.5">Password</label>
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(p => !p)} tabIndex={-1} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                  {showPassword
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="reg-confirm" className="block text-sm font-semibold text-gray-600 mb-1.5">Konfirmasi Password</label>
              <div className="relative">
                <input
                  id="reg-confirm"
                  ...
                  className={`w-full bg-gray-50 border-2 focus:ring-4 rounded-2xl px-4 py-3 pr-11 text-sm font-medium text-gray-800 outline-none transition-all placeholder:text-gray-300 ${
                    passwordMismatch
                      ? 'border-red-300 focus:border-red-400 focus:ring-red-100'
                      : passwordMatch
                      ? 'border-green-300 focus:border-green-400 focus:ring-green-100'
                      : 'border-gray-100 focus:border-brand-blue focus:ring-brand-blue/10'
                  }`}
                />
                <button type="button" onClick={() => setShowConfirm(p => !p)} tabIndex={-1} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                  {showConfirm
                    ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" /></svg>
                    : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  }
        ...
        <p className="text-center text-sm text-gray-500 mt-5">
          Sudah punya akun?{' '}
          <Link to="/auth/login" className="text-brand-blue font-bold hover:underline">Masuk di sini</Link>
        </p>
      </div>
    </div>
  );
